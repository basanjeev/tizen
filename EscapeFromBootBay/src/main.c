...
static void
_relto_del_cb(void *data, Evas *e , Evas_Object *obj ,
              void *event_info )
{
...
static void
_target_dec(void *data, EPhysics_Body *body, void *event_info )
{
   Game *game = data;
...
static void
_sea_of_death(void *data, EPhysics_Body *body, void *event_info )
{
   Game *game = data;
   double body_mass;
   int y;
...
static void
_switch_sound_cb(void *data, Evas_Object *o ,
...
static void
_extra_obj_del_cb(void *data, Evas_Object *obj, const char *emission , const char *source )
{
   Game *game = data;
...
_boom_add(Game *game, int x, int y, int pts)
{
   ...
   edje_object_file_set(boom, path, "boom");
...
static void
_camera_moved_cb(void *data, EPhysics_World *world ,
                 void *event_info )
{
   Evas_Object *object;
   Game *game = data;
...
static void
_body_camera_limit(void *data, EPhysics_Body *body, void *event_info )
{
...
static void
_body_update(void *data , EPhysics_Body *body,
             void *event_info )
{
...
static void
_mouse_move(void *data, Evas *e , Evas_Object *obj ,
            void *event_info)
{
...
static void
_mouse_up(void *data, Evas *e , Evas_Object *obj ,
          void *event_info)
{
   Game *game = data;
   EPhysics_Body *body;
   ...
   char path[1024]; snprintf(path, 1024, "%s/bomb.png", app_get_resource_path());
   evas_object_image_file_set(body_image, path, NULL);
   err = evas_object_image_load_error_get(body_image);
   if (err != EVAS_LOAD_ERROR_NONE)
       WRN("could not load image : %s",  "/bomb.png", evas_load_error_str(err));
...
static void
_mouse_down(void *data, Evas *e , Evas_Object *obj ,
            void *event_info)
{
   Evas_Event_Mouse_Down *mouse_info = event_info;
   Game *game = data;
...
static void
_mouse_move_bg(void *data, Evas *e , Evas_Object *obj ,
               void *event_info)
{
   Evas_Event_Mouse_Down *mouse_info = event_info;
   Game *game = data;
...
static void
_mouse_up_bg(void *data, Evas *e , Evas_Object *obj ,
             void *event_info)
{
   Game *game = data;
   Evas_Event_Mouse_Up *mouse_info = event_info;
...
static void
_mouse_down_bg(void *data, Evas *e , Evas_Object *obj ,
               void *event_info)
{
   Evas_Event_Mouse_Down *mouse_info = event_info;
   Game *game = data;
             ...
             float mass, int relto)
{
   ...
   snprintf(image, sizeof(image), "%s/%s_%i_%i.png", app_get_resource_path(), type, w, h);
...
_create_target(Game *game, const char *type, int w, int h, int x, int y)
{
   ...
   body_data->hp = 130;
   snprintf(image, sizeof(image), "%s/%s.png", app_get_resource_path(), type);
               ...
               int above)
{
   ...
   char path[1024]; snprintf(path, 1024, "%s/etrophy.edj", app_get_resource_path(), GAME_THEME);
   edje_object_file_set(object_image, path, type);
...
static void
_menu(void *data, Evas_Object *layout, const char *emission ,
          const char *source )
{
   Game *game = data;
...
static void
_pause_game_cb(void *data, Evas_Object *layout ,
               const char *emission , const char *source )
{
   Game *game = data;
   ...
   char path[1024]; snprintf(path, 1024, "%s/etrophy.edj", app_get_resource_path(), GAME_THEME);
   elm_layout_file_set(pause_layout, path, "pause_win");
...
_options_layout_add(Game *game)
{
   ...
   char path[1024]; snprintf(path, 1024, "%s/etrophy.edj", app_get_resource_path(), GAME_THEME);
   elm_layout_file_set(options_layout, path, "options");
...
static void
_level_load_cb(void *data, Evas_Object *layout, const char *emission ,
          const char *source )
{
   Game *game = data;
...
_retry_layout_add(Game *game, Eina_Bool win)
{
   ...
   char path[1024]; snprintf(path, 1024, "%s/etrophy.edj", app_get_resource_path(), GAME_THEME);
   elm_layout_file_set(retry_layout, path, buffer);
...
static void
_wstopped(void *data, EPhysics_World *world ,
          void *event_info )
{
   Evas_Object *retry_layout;
   EPhysics_Body *body;
...
_levels_load(Game *game)
{
   ...
   char path[1024]; snprintf(path, 1024, "%s/levels.eet", app_get_resource_path());
   game->world = world_load(path);
   if (!game->world)
     {
        ERR("Failed to load world from /levels.eet");
        return EINA_FALSE;
     }
   ...
   if (!game->levels)
     {
        ERR("Failed to load levels from /levels.eet");
        return EINA_FALSE;
     }
