...
/**
 * @typedef Etrophy_Level_Format_Cb
 *
 * Returns a format as a litteral string that is used to compose an
 * internationalized group of words to describe a level.
 *
 * For instance, a racing game would like to call the levels 'stage' and
 * make it change in function of the name of the level. One might also want
 * to place the description before or after the "Level" word.
 *
 * @ingroup Score
 */
typedef const char *(*Etrophy_Level_Format_Cb)(const char *level);


/**
 * @brief
 * Set the callback that returns the custom name of a 'Level'.
 * Use @ref Etrophy_Level_Name_Cb to customize 'Level'.
 *
 * @param callback The callback to set. Cannot be NULL.
 *
 * @ingroup Score
 */
EAPI void etrophy_level_format_cb_set(Etrophy_Level_Format_Cb callback);
 ...
 * @code
 * static void
 * _scores_show_cb(void *data, Evas_Object *obj ,
 *                 void *event_info )
 * {
