...
#define NUM_ITEMS             6
#define NUM_INDEX             6
#define NUM_ITEMS_CIRCLE_EVEN 7
#define NUM_INDEX_CIRCLE_EVEN 7
...
static void
_thing_clicked_cb(void *data, Evas_Object *obj EINA_UNUSED, void *event_info EINA_UNUSED)
{
	appdata_s *ad = (appdata_s *)data;
	dlog_print(DLOG_INFO, LOG_TAG, "_thing_clicked_cb ad %p RVC %d ", ad, ad->master.data.rvc.count);
	if(scene_changed == 2){
		...
		if(!path) return;
		for(i = 0; i<NUM_ITEMS_CIRCLE_EVEN; i++){
			sprintf(buf, "/t%d.png", i+1);
			if(strstr(path, buf)){
				dlog_print(DLOG_INFO, LOG_TAG, "Checking %s in %s", buf, path);
				ad->master.res = get_res_by_devtype((devtype_e)i);
				break;
			}
		}
		if(i==NUM_ITEMS_CIRCLE_EVEN){
			dlog_print(DLOG_INFO, LOG_TAG, "FIXME : Check Why");
			return;
		}
		char msg[1024];
		snprintf(msg, 1023, "Executing Thing %d", ad->master.res->devtype);
		dlog_print(DLOG_INFO, LOG_TAG, msg);
		eext_rotary_selector_cb(ad, obj, NULL);
	}else if(scene_changed == 1){
		dlog_print(DLOG_INFO, LOG_TAG, "Ready state");
		scene_changed = 2;
...
static void
_scene_clicked_cb(void *data, Evas_Object *obj EINA_UNUSED, void *event_info EINA_UNUSED)
{
	int scene = 0;
	if(scene_changed == 2){
...
#ifdef OCF
		lr_s *ptr = get_livingroom_thing(MSP4);
		//if(!ptr->data.b) //  if state is true - scene in progress
		{
			ptr->data.i = scene;
			ptr->data.b = true;
			putOCF(ptr);
...
#endif
		char msg[1024];
		snprintf(msg, 1023, "Executing Scene %d", scene);
		dlog_print(DLOG_INFO, LOG_TAG, msg);
		//show_graphic_popup(ad, msg, path, 5);
	}else if(scene_changed == 1){
		dlog_print(DLOG_INFO, LOG_TAG, "Ready for Scene %d", scene);
		scene_changed = 2;
	}else if(scene_changed == 0){
		scene_changed = 1;
	}
...
_create_view_layout(void *data, char *layout_file, char *index_style, char it_style[][20])
{
	...
	for (i = 0; i < max_items; ++i)
	{
		...
		if(!strcmp("Things", choice)){
			snprintf(img_path, sizeof(img_path), "%s/things/t%d.png", ICON_DIR, (i % max_items) + 1);
			evas_object_smart_callback_add(img, "clicked", _thing_clicked_cb , ad);
		}else{
			snprintf(img_path, sizeof(img_path), "%s/scenes/s%d.jpg", ICON_DIR, (i % max_items) + 1);
			evas_object_smart_callback_add(img, "clicked", _scene_clicked_cb , img);
		}
...
_body_circle_even_queue_cb(void *data, Evas_Object *obj, void *event_info)
{
	...
	_create_view_layout(ad, "elmdemo-test/index_circle", "circle", it_style);
}
...
_body_circle_odd_queue_cb(void *data, Evas_Object *obj, void *event_info)
{
	...
	_create_view_layout(nf, "elmdemo-test/index_circle", "circle", it_style, "");
}
