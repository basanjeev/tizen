...
static void
_gesture_clicked_cb(void *data, Evas_Object *obj EINA_UNUSED, void *event_info EINA_UNUSED)
{
#if 0
	...
	if(scene_changed == 2){
		...
		char msg[1024];
		snprintf(msg, 1023, "Executing Gesture %d", ad->master.res->devtype);
...
_scene_clicked_cb(void *data, Evas_Object *obj EINA_UNUSED, void *event_info EINA_UNUSED)
{
	...
	if(scene_changed == 2){
		...
		dlog_print(DLOG_INFO, LOG_TAG, msg);
		show_graphic_popup(ad, msg, 10);
		ad->master.restype = USERPROFILE;
		postOCF(&ad->master);
#endif
	}else if(scene_changed == 1){
...
_create_view_layout(void *data, char *layout_file, char *index_style, char it_style[][20])
{
	...
	for (i = 0; i < max_items; ++i)
	{
		...
		if(!strcmp("SelectUser", choice)){
			snprintf(img_path, sizeof(img_path), "%s/things/t%d.png", ICON_DIR, (i % max_items) + 1);
			evas_object_smart_callback_add(img, "clicked", _scene_clicked_cb , ad);
		}else{
			snprintf(img_path, sizeof(img_path), "%s/scenes/s%d.png", ICON_DIR, (i % max_items) + 1);
			evas_object_smart_callback_add(img, "clicked", _scene_clicked_cb , ad);
		}
